...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {
        # domains are represented by coord tuples for Pacman and each ghost
        ...
        # observation range is a combination of two things:
        #   -   max manhattan distance (x+y-2)
        #   -   noise (noise+1)
        # so the end result is x+y+noise-1
        OBS0: list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1)),
        OBS1: list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))
    }

    "*** YOUR CODE HERE ***"
    # all my code was written above in the three variables, so nothing goes here
    # raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # get sum of values in distribution via total()
        total = self.total()
        # no need to normalize if total key sum == 0
        if total > 0:
            # normalize each distribution key
            # this makes the total key sum == 1
            for key in self.keys(): self[key] /= total
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # again, no need if total key sum == 0
        if self.total() == 0: return None
        # pick random value to be iteration limit
        limit = random.random() * self.total()
        sum = 0
        # iterate through distribution keys, add key values to sum
        # when limit is broken by sum, return associated key
        for key, val in self.items():
            if sum + val >= limit: return key
            sum += val
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # jailed ghosts are handled separately
        if ghostPosition == jailPosition:
            return (1 if noisyDistance is None else 0)
        # aside from that case, prob = 0 when there's no noisy distance
        if noisyDistance is None: return 0

        # manhattan distance between pacman and ghost gets us true distance
        trueDistance = manhattanDistance(pacmanPosition, ghostPosition)
        # get observation probability via busters' obsprob method w/ noisy and true distances
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # position of jail
        pos_jail = self.getJailPosition()

        # iterate through each ghost's position
        for pos in self.allPositions:
            # take obsprob from comparing positions of Pacman and ghost
            obsprob = self.getObservationProb(observation, pos_Pacman, pos, pos_jail)
            # apply obsprob ratio to belief of position
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # new beliefs of time step hold a seperate distribution
        beliefs_new = DiscreteDistribution()
        # iterate through all ghost positions in current state
        for pos_old in self.allPositions:
            # find new position distribution of ghost
            posDist_new = self.getPositionDistribution(gameState, pos_old)
            # then iterate through each position in said distribution
            for pos_new, prob in posDist_new.items():
                # update belief of new position w/ probability that ghost moves to said new position
                beliefs_new[pos_new] += self.beliefs[pos_old] * prob
        # make sure new beliefs add up to 1
        beliefs_new.normalize()
        # make new beliefs current beliefs, cycle continues in next call
